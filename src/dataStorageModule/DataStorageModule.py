...
class DatabaseModule(threading.Thread):
    def __init__(self, db_path="cyber_trainer.db"):
        ...
        self.daemon = True
        # Nie otwieramy połączenia tutaj! Przenosimy je do run()
        self.conn = None
    ...
    def run(self):
        ...
        # Otwieramy połączenie tutaj – wewnątrz docelowego wątku roboczego
        ...
        try:
            ...
            # Zamykamy połączenie w tym samym wątku, w którym powstało
            if self.conn:
                self.conn.close()
